...
class CustomTournamentController extends Controller
{
    ...
    public function updateMatch(Request $request, string $slug, int $roundId, int $matchId): JsonResponse
    {
        ...
        $data = $request->validate([
            ...
            'venue'        => 'nullable|string|max:200',
            'status'       => 'sometimes|in:scheduled,in_progress,finished,cancelled,postponed,suspended,paused,rescheduled',
        ]);
    ...
    public function updateMatchStatus(Request $request, string $slug, int $roundId, int $matchId): JsonResponse
    {
        ...
        $request->validate([
            'status' => 'required|in:scheduled,in_progress,finished,cancelled,postponed,suspended,paused,rescheduled',
        ]);
